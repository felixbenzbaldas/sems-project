...
<template>
    <div v-if="!hidden" style="display: inline">
        <div v-if="identity.appA?.ui?.commands">
            <VueUI :identity="identity.appA?.ui.commands" :is-view="true"/>
        </div>
        <VueUI v-if="identity.appA?.ui" :identity="props.identity.appA.ui.output.getUi()" :is-view="true"></VueUI>
        <VueUI v-if="identity.appA?.ui && !identity.appA.ui.isWebsite" :identity="props.identity.appA.ui.input.getUi()" :is-view="true"></VueUI>
        <div v-if="identity.appA?.ui && !identity.appA.ui.isWebsite" style="margin-bottom: 0.5rem; padding-bottom: 0.5rem; border-bottom: dashed"></div>
        <VueUI v-if="identity.appA?.ui" :identity="identity.appA.ui.content" :is-view="identity.appA.ui.isWebsite"/>
